...
\[
\RR = \{j \in \HH : p^{D^*}_j > p^{sup}_i \text{,  } d_{geo}(i,j) \leq r_{inner}\},
\]
...
Una última consideración que hay que tener son los casos extremos. Un caso es que la zona del hotel buscado tenga muy poca población y el otro es cuando el hotel es muy barato para la zona, por lo que el filtro de precio le impide ver suficientes de los demás hoteles. Para aliviar el primer problema el procedimiento es simple: como no queremos que se queden vacías las recomendacio

Si en el cálculo de $r_{inner}$ resulta que hay muy poco hoteles porque
