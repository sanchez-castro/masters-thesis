...
-- =============================================
ALTER PROCEDURE [dbo].spRM_ObtenerRecomendacionesHoteles @clav_hotel INT
	,@fecha DATE
AS
BEGIN
	...
	IF (@nrec = 0)
	BEGIN -- NO HAY RECOMENDACIONES DEL ALGORITMO NUEVO
		...
		SET @estrellas = (
				SELECT Clav_Categoria_Maletas
				FROM @datos_hotel
				)

		SELECT Clav_HotelRecomendacion
			,ROW_NUMBER() OVER (
				ORDER BY Posicion
				) AS Posicion
		FROM (
			SELECT DISTINCT TOP 50 h.Clav_Hotel AS Clav_HotelRecomendacion
				,CASE 
					WHEN h.Clav_Hotel = @clav_hotel
						THEN 0
					ELSE ISNULL(so.posicion, 1000000)
					END AS Posicion
			FROM Hoteles h
			INNER JOIN hoteles_Destinos hd ON h.Clav_Hotel = hd.clav_hotel
			LEFT JOIN (
				SELECT so2.clav_hotel
					,so2.clav_destino
					,so2.posicion
				FROM BI_SortOrder so2 WITH (NOLOCK)
				WHERE so2.FechaInicio <= @fecha
					AND @fecha < so2.FechaFinal
					AND so2.clav_destino = @destino
				) so ON h.Clav_Hotel = so.Clav_Hotel
	...
	END
	ELSE
	BEGIN -- SÍ HAY RECOMENDACIONES DEL ALGORITMO NUEVO
		SELECT TOP 50 Clav_HotelRecomendacion
			,Prioridad AS Posicion
		FROM RM_Hoteles_HotelesRecomendaciones WITH (
				NOLOCK
				,INDEX (IX_RM_Hoteles_HotelesRecomendaciones_Clav_Hotel)
				)
		WHERE Clav_Hotel = @clav_hotel
