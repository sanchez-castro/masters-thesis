...
$$
div(x, y) = \frac{1}{2} \sum_k |\frac{x_k}{\sum_j x_j} - \frac{y_k}{\sum_j y_j}|
$$
...
2. Calcular los candidatos para un hotel dado:
    1. Retener los candidatos iniciales:
        ...
        * Los hoteles que quedan después de este filtro son los candidatos iniciales.
...
3. Generar recomendaciones para el hotel en un rango de fechas dado:
    1. Si se utilizó el criterio de $k_{min}$ en el paso 2.1, entonces:
        * Regresar esos hoteles, sin importar su precio.
    2. En otro caso:
        * Filtrar los candidatos demasiado caros en la fecha en la que se quiere reservar.
        * Regresar los $k$ candidatos dentro de la cerca interior con mayor _score_.


### Notas adicionales

Dado que el objetivo de este sistema es recomendar hoteles parecidos _y_ cercanos, no hay pérdida de precisión al ignorar los hoteles fuera de la cerca exterior a la hora de calcular la cerca interior, ni al momento de elegir las recomendaciones.

Con el fin de mejorar el desempeño, se almacena únicamente las primeras recomendaciones, con algunas de más para el caso de que las mejores se llenen o no esten disponibles. Si se quiere quedar con digamos $r$ recomendaciones, se toman los primeros $R$ hoteles, donde $R$ es el mínimo número de hoteles dentro de la cerca interior que se tiene que tomar de modo que $r$ cumplan el filtro de precio de largo plazo. La idea es garantizar que independientemente de las fluctuaciones de precio, se tengan al menos $r$ hoteles a recomendar.
